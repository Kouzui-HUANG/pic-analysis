...
> 一套純前端、無依賴的專業級影像分析引擎，提供**影像診斷與調色**、**配色方案引擎**、**參考圖仿調色**三種模式。透過多階段管線自動診斷影像特性、識別藝術場景意圖，並以像素級精度施加保護性修正；可一鍵套用 9 種色彩和諧配色方案（含原始配色對照），搭配膚色／髮色保護、備選色相、智慧飽和度、AI 配色與自動校正；亦可上傳參考圖，以三層（LAB 統計轉移 + 直方圖塑形 + 色相對齊）管線將目標圖的色系、光暗、濃度貼近參考圖。
...
3. [各模組詳細介紹](#3-各模組詳細介紹)
   ...
   - [transfer.js — 參考圖仿調色引擎](#38-transferjs--參考圖仿調色引擎)
   - [lang.js — 多語系模組](#39-langjs--多語系模組)
   - [main.js — 應用程式協調器](#310-mainjs--應用程式協調器)
...
**ToneLab**（調色研究所）是一套以瀏覽器為執行環境的智慧影像分析與自動增強系統，專為攝影師與影像創作者設計。系統提供三種核心模式：

- **影像診斷與調色（ToneLab）**：透過四階段管線自動診斷影像特性、識別藝術場景意圖，並以像素級精度施加保護性修正
- **配色方案引擎（Recolor）**：提供 9 種配色方案（含原始配色對照），搭配膚色／髮色保護、備選色相、智慧飽和度滑桿、AI 配色（Gemini）與自動後校正
- **參考圖仿調色（Reference Match / Transfer）**：上傳目標圖與參考圖，系統以「LAB 統計轉移 + 直方圖塑形 + 色相對齊」三層管線將目標圖的色系、光暗、濃度貼近參考圖，並支援膚色保護與 AI 仿調色
...
| **純前端** | 所有運算在瀏覽器端完成，影像資料不會上傳至伺服器（AI 模式需外呼 Gemini） |
| **三入口首頁** | 配色方案引擎（Recolor）／參考圖仿調色（Transfer）／影像診斷與調色（ToneLab）並列 |
| **雙處理模式** | ToneLab 提供攝影模式（修正偏差）與插畫模式（強化風格）可自由切換 |
| **場景感知** | 自動偵測 14 種藝術場景類型，防止破壞創作者意圖 |
| **像素級保護** | 每種調整皆有獨立的邊緣保護輪廓，避免 posterization 與色彩雜訊 |
| **雙向調整** | 系統可對影像進行增強或抑制，依實際需求雙向修正 |
| **智慧配色** | 9 種配色方案（含原始配色），支援膚色／髮色保護、備選色相與自動校正 |
| **智慧飽和度** | Recolor 內建 -100～+100 飽和度滑桿，依處理模式自動估算方向（攝影=抑制、插畫=增強）|
| **參考圖仿調色** | 三層引擎（Reinhard LAB 轉移、Histogram Matching、色相對齊）+ 膚色保護 + 細節保留 |
| **AI 配色/仿調色** | 可選用 Gemini API，保留膚色與髮色不變；支援左右滑動比較（演算法 ↔ AI） |
| **API Key 管理** | 內建 Modal：輸入、顯示/隱藏切換、可選擇儲存至本地檔案並自訂路徑 |
| **管線快取** | ToneLab 於背景預先運算另一模式結果，模式切換零延遲 |
| **雙語界面** | 支援英文與繁體中文，所有界面元素完整在地化（含參考圖仿調色新增項目） |
| **預設系統** | ToneLab 支援 5 組參數預設值，透過 localStorage 持久保存 |

---

## 2. 整體架構說明

ToneLab 採用**三軌管線架構**——使用者從首頁三張並列卡片中選擇「配色方案引擎（Recolor）」、「參考圖仿調色（Transfer）」、或「影像診斷與調色（ToneLab）」。三條管線共享基礎模組（color.js、stats.js、lang.js）與診斷管線（Analyzer → Scene → Strategy → Adjuster）；Recolor 額外引入配色引擎（recolor.js），Transfer 額外引入仿調色引擎（transfer.js）並重用 recolor.js 的 `skinScore` 與 `extractPalette`。ToneLab 管線支援**背景預算另一模式**，實現模式切換零延遲。
...
### 管線 C：參考圖仿調色（Transfer / Reference Match）

```
┌─────────────────────────────────────────────────────────────────┐
│      使用者第一步上傳「目標圖」，第二步上傳「參考圖」             │
└─────────────────────────────┬───────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│              buildProfile(refImageData)                          │
│  LAB 統計（mean/std） + 亮度 CDF + K-means 調色盤 + 主色相        │
│  最多 200,000 樣本抽樣，結果可快取（transferRefProfile）         │
└─────────────────────────────┬───────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│              Transfer.match(targetImageData, profile, opts)      │
│                                                                  │
│  ⟡ Layer 1（LAB 統計轉移，Reinhard）                             │
│    Lnew = (L - μ_s) × (σ_t / σ_s) + μ_t，a/b 通道同式            │
│    控制項：lumStrength、colorStrength                            │
│                                                                  │
│  ⟡ Layer 2（直方圖塑形 Histogram Matching）                      │
│    在 L 通道上依 CDF 建 LUT，微調陰影/中間調/亮部分布           │
│    控制項：histogramShape                                        │
│                                                                  │
│  ⟡ Layer 3（色相對齊 Hue Alignment）                             │
│    將目標主色相映射至參考主色相，Gaussian 衰減平滑混合           │
│    RGB 軸色相旋轉（與 Recolor 相同矩陣），skinScore 膚色保護     │
│    控制項：hueStrength、skinProtect                              │
│                                                                  │
│  ⟡ 最終混合：overall × (1 - detailRetain) 對原圖                 │
│    控制項：overallStrength、detailRetain                         │
└─────────────────────────────┬───────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│                    渲染結果（main.js）                           │
│  - 三欄並列：目標原圖 / 參考圖 / 仿調色結果                      │
│  - 參考圖摘要（主色盤、主色相、平均 LAB）                        │
│  - 三方診斷對照（目標 / 參考 / 結果）                            │
│  - 可選 AI 仿調色 + 左右滑動比較（演算法 ↔ AI）                 │
│  - 可收合進階分析（參考圖場景、三方診斷、主色盤對照）            │
└─────────────────────────────────────────────────────────────────┘
```
...
### 3.8 transfer.js — 參考圖仿調色引擎

Reference Match（參考圖仿調色）模組，將「目標圖」的色系、亮暗、濃度貼近一張「參考圖」。採用三層獨立強度控制的管線，各層可單獨歸零。

#### 三層架構

| 層 | 演算法 | 作用通道 | 作用 |
|---|-------|---------|------|
| **Layer 1** | Reinhard LAB 統計轉移 | L / a / b | 匹配平均亮度、對比度與整體色彩偏向 |
| **Layer 2** | Histogram Matching（LUT） | L | 微調陰影/中間調/亮部分布形狀 |
| **Layer 3** | RGB 軸色相旋轉 + 色相對齊 | H（HSL） | 將目標主色相映射至參考主色相（含膚色保護） |

#### 色相對齊機制

1. `buildProfile(refImageData)` 從參考圖萃取：LAB 統計（mean/std）、亮度 256-bin CDF、K-means 調色盤（k=6）、加權主色相陣列
2. `buildSourceStats(targetImageData)` 從目標圖計算同樣的統計（不含 palette 權重）
3. `buildHueMap(srcDoms, tgtDoms)`：對每個目標主色相，找出最近的參考主色相，記錄 signed shift 與權重
4. `pixelHueShift(h, hueMap)`：對每個像素色相，依 Gaussian 衰減（σ=0.10，約 36°）加權融合各錨點位移
5. 最終 RGB 軸色相旋轉與 `Recolor.applyScheme` 共用同一旋轉矩陣

#### 膚色保護

- 重用 `Recolor.skinScore(hue, sat, lum)` 偵測膚色像素
- 同時對 Layer 3（色相位移）與 Layer 1（LAB 統計轉移）施加保護：膚色像素保留原始 L*、a*、b* 比例
- `skinProtect` 滑桿（0-100%）控制保護力度

#### 7 個可調控制項

| 滑桿 | 範圍 | 預設 | 作用 |
|------|------|------|------|
| `overallStrength` | 0-1 | 1.0 | 最終與原圖的混合比例 |
| `lumStrength` | 0-1 | 0.8 | Layer 1 的 L 通道強度（光暗匹配） |
| `colorStrength` | 0-1 | 0.8 | Layer 1 的 a/b 通道強度（色彩匹配） |
| `hueStrength` | 0-1 | 0.6 | Layer 3 色相對齊強度 |
| `histogramShape` | 0-1 | 0.3 | Layer 2 直方圖塑形強度 |
| `skinProtect` | 0-1 | 0.5 | 膚色保護強度（色相 + LAB 同步生效） |
| `detailRetain` | 0-1 | 0 | 保留目標原圖細節比例（與 overall 相乘衰減） |

#### 公開 API

```javascript
// 從參考圖建立 Profile（LAB 統計 + 亮度 CDF + 主色盤 + 主色相）
const profile = PicAnalysis.Transfer.buildProfile(refImageData): Profile

// 從目標圖建立 Source Stats（結構類似 Profile，用於內部 LUT 建置）
const srcStats = PicAnalysis.Transfer.buildSourceStats(targetImageData): SourceStats

// 執行三層仿調色，回傳新的 ImageData
const result = PicAnalysis.Transfer.match(
  targetImageData: ImageData,
  profile: Profile,
  options?: {
    overallStrength, lumStrength, colorStrength,
    hueStrength, histogramShape, skinProtect, detailRetain
  }
): ImageData
```
...
- 統計階段最多 200,000 樣本抽樣（對 4K 影像約跳步 4）
- 單次 pass 同時套用三層（Layer 3 → Layer 1 → Layer 2 合併），避免多次 LAB 往返
- Profile 可快取於 `transferRefProfile`，變更滑桿只需重跑像素迴圈
...
### 3.10 main.js — 應用程式協調器

負責串接所有模組、管理 UI 互動、以及協調三條處理管線。

#### 主要職責

| 功能 | 說明 |
|------|------|
| **三入口首頁** | 左：配色方案引擎（Recolor）／中：參考圖仿調色（Transfer）／右：影像診斷與調色（ToneLab），各有獨立拖放區 |
| **影像載入** | 支援拖放（drag & drop）與點擊上傳，縮放至最大處理尺寸（MAX_PROCESS_DIM = 2048） |
| **ToneLab 管線** | Analyzer → Scene → Strategy → Adjuster；參數變更跳過 Stage 1 |
| **管線快取** | `pipelineCache[mode]` 儲存 Stage 2+3 結果；模式切換後背景 `setTimeout(0)` 預算另一模式 |
| **Recolor 管線** | 產生 9 種配色方案、膚色保護、備選色相方案、智慧飽和度、自動後校正 |
| **Transfer 管線** | `runTransferPipeline()` 呼叫 `Transfer.match()`；快取 `transferRefProfile`、`transferResultData`、滑桿去彈 `transferRerunTimer` |
| **智慧飽和度** | `computeAutoVibrance(satMean, satStd, mode)` 依模式自動估算方向（攝影=負、插畫=正）；`recolorVibranceUserSet` 旗標保護使用者手動設定 |
| **角色色彩偵測** | `extractCharacterColors(canvas)` 提取膚色 + skinShadow + skinHighlight；AI 配色時由 Gemini 辨識髮色 |
| **備選方案** | `renderAlternativePalettes(idx)` 產生 2 個備選色相（+120°/+240°），`applyVariant(i)` 一鍵套用 |
| **AI 配色** | `aiAnalyzeImage()` + `aiRecolorImage()`，呼叫 Gemini 視覺模型，保留膚色/髮色不變；`formatCharacterColorsForPrompt()` 注入 CRITICAL 保護指令 |
| **AI 仿調色** | Transfer 模式下亦可觸發 `#transfer-ai-btn`，結果存於 `transferAiResultData` |
| **AI 比較檢視** | 左右滑動分割條（draggable `.ai-compare-divider`）比較「演算法」與「AI」結果，Recolor 與 Transfer 皆支援 |
| **API Key 管理** | Modal 包含輸入、顯示/隱藏切換、儲存至本地檔案 checkbox、儲存路徑選擇（預設桌面）、從檔案載入 |
| **處理模式切換** | 攝影模式 ↔ 插畫模式即時切換（利用 `pipelineCache` 零延遲） |
| **直方圖渲染** | 分別為原始與調整後影像繪製亮度與 RGB 四格直方圖 |
| **診斷面板** | 顯示診斷結果（含原圖 vs 調整後並列對照）及進度條；Transfer 為三方（目標/參考/結果） |
| **場景標籤** | 顯示偵測到的場景及其信心分數（含視覺信心條） |
| **調整列表** | 列出所有套用的調整及原因說明 |
| **參數控制** | 滑桿控制項，即時更新並重跑管線 |
| **預設系統** | 儲存/載入/重設 5 組命名預設值（localStorage：`picanalysis_presets`） |
| **語言切換** | 即時切換英文/繁體中文 |
| **可收合進階分析** | Recolor 與 Transfer 模式皆有 `.recolor-analysis-collapsible`，預設收合，減少視覺複雜度 |
| **處理中動畫** | `#processing-overlay` 以像素咖啡場景動畫覆蓋長時運算（AI 呼叫、初始管線） |
| **上下文按鈕** | 上傳/下載按鈕隨模式動態調整標籤（如「下載 (AI)」），Landing 頁面隱藏非必要控制項 |
| **下載** | 將調整後/配色後/仿調色後影像輸出為 PNG 檔案 |
...
#### 智慧飽和度（Smart Vibrance）

Recolor 工作區提供 `-100 ～ +100` 的智慧飽和度滑桿。上傳新圖時系統呼叫 `computeAutoVibrance(satMean, satStd, mode)`，依下列規則估算初始值：

```
sign = (mode === PHOTO) ? -1 : +1      // 攝影=抑制，插畫=增強
base = (satMean - 0.30) × 150           // 偏離 0.30 的絕對差
variety = (satStd - 0.08) × 100         // 色彩多樣性加成
value = clamp(sign × (base + variety), -50, +50)
```

若使用者手動調整滑桿，`recolorVibranceUserSet` 旗標會置為 true，之後切換處理模式或重跑管線時不再自動覆寫。

### 4.8 參考圖仿調色機制

Transfer 引擎的核心理念是「**以統計量對齊，避免逐像素生成式計算**」——讓目標圖的整體分布（平均、標準差、直方圖形狀、主色相）貼近參考圖，同時保留目標圖的構圖與細節。

#### 為何採用三層而非單層

| 僅 LAB 轉移 | 僅直方圖匹配 | 僅色相旋轉 | 三層組合 |
|------------|-------------|-----------|---------|
| 平均亮度/色彩接近，但陰影/亮部細節分布不一致 | 亮度分布逼近，但色彩偏向完全無法轉移 | 色調方向正確，但亮暗濃度完全保留目標 | 三者互補，各自可獨立歸零 |
...
```
L_new = (L - μ_src) × (σ_target / σ_src) + μ_target
```

此公式先將像素「標準化」至零均值單位標準差，再縮放至參考圖的統計量。天然保留局部細節（因為只動統計量，不動相對關係）。

#### 直方圖匹配 LUT

對 L 通道建立 256-bin 直方圖並累積為 CDF（Cumulative Distribution Function），找出每個來源 bin 對應的目標 bin：

```
LUT[i] = argmin_j |tgtCDF[j] - srcCDF[i]|
```

這修正了 Reinhard 僅能對齊前兩個矩的局限——例如目標圖陰影較少但參考圖陰影豐富時，LUT 會將目標的暗部值壓暗更多。

#### 色相對齊的 Gaussian 衰減

對目標圖每個主色相，找最近的參考主色相，紀錄 signed shift。對每個像素，依該像素色相與各錨點的距離，以 Gaussian (σ=0.10，約 36°) 加權融合所有位移：

```
shift(h) = Σ shift_i × weight_i × exp(-d_i² / 2σ²) / Σ weight_i × exp(-d_i² / 2σ²)
```

此方式避免「色相跳變」——鄰近錨點的像素獲得錨點 shift，遠離所有錨點的像素獲得小位移或零。
...
### Transfer 執行序列（Reference Match）

```
使用者上傳「目標圖」至 Transfer 卡片
  │
  ▼
loadTransferTarget()
  ├── 縮放目標圖並快取 transferTargetImageData
  └── 顯示 Transfer 工作區，等待參考圖

使用者上傳「參考圖」（或點擊更換參考圖）
  │
  ▼
loadTransferReference()
  ├── 縮放參考圖並快取 transferRefImageData
  ├── PicAnalysis.Transfer.buildProfile(transferRefImageData)
  │   → 快取至 transferRefProfile（LAB 統計 + 亮度 CDF + 主色盤 + 主色相）
  ├── PicAnalysis.Analyzer.analyze(refImageData) → transferRefDiagnosis
  ├── PicAnalysis.Scene.detect(transferRefDiagnosis) → transferRefScenes
  └── 呼叫 runTransferPipeline()

runTransferPipeline()
  ├── PicAnalysis.Transfer.match(transferTargetImageData, transferRefProfile, opts)
  │   → transferResultData
  ├── PicAnalysis.Analyzer.analyze(transferResultData) → transferResultDiagnosis
  ├── 渲染三欄（目標 / 參考 / 結果）+ 三方診斷對照 + 主色盤對照
  └── 進階分析面板（預設收合）
...
scheduleTransferRerun()
  └── transferRerunTimer 去彈後重跑 runTransferPipeline()
      （profile 已快取，只重跑 Transfer.match 像素迴圈）

AI 仿調色（可選）
  │
  ▼
#transfer-ai-btn → aiRecolorImage(...) → transferAiResultData
  └── 左右滑動比較（#transfer-ai-compare-divider）
```
...
### PicAnalysis.Transfer

```javascript
// 從參考圖建立 Profile（LAB mean/std、亮度 CDF、K-means 調色盤、主色相陣列）
const profile = PicAnalysis.Transfer.buildProfile(refImageData: ImageData): Profile
// Profile = {
//   labStats: { L: { mean, std }, a: { mean, std }, b: { mean, std } },
//   lumCDF:   Float64Array(256),
//   palette:  Palette[],
//   dominantHues: Array<{ hue: number, weight: number }>
// }

// 從目標圖建立 Source Stats（結構類似 Profile，用於內部 LUT 建置）
const srcStats = PicAnalysis.Transfer.buildSourceStats(targetImageData: ImageData): SourceStats

// 執行三層仿調色（Reinhard LAB → Histogram → Hue Alignment）
const result = PicAnalysis.Transfer.match(
  targetImageData: ImageData,
  profile: Profile,
  options?: {
    overallStrength?: number,   // 0-1，預設 1.0
    lumStrength?: number,       // 0-1，預設 0.8
    colorStrength?: number,     // 0-1，預設 0.8
    hueStrength?: number,       // 0-1，預設 0.6
    histogramShape?: number,    // 0-1，預設 0.3
    skinProtect?: number,       // 0-1，預設 0.5
    detailRetain?: number       // 0-1，預設 0.0
  }
): ImageData
```
...
4. **備選方案**：每種方案下方顯示 2 個備選色相方案（+120°/+240°），點擊即可套用
5. **調整強度**：使用強度滑桿（0-100%）控制配色效果的混合比例
6. **膚色保護**：使用膚色保護滑桿（0-100%）控制膚色區域的保護力度
7. **智慧飽和度**：使用 `-100 ～ +100` 滑桿微調飽和度；系統會依處理模式給初始建議值（攝影=負、插畫=正），手動拖動後不再覆寫
8. **自訂基色相**：使用基色相滑桿（0-360°）手動指定配色基準色相，或按「Auto」自動偵測
9. **查看對比**：色盤對比區顯示原始色盤與新配色方案的色彩對照，並顯示偵測到的膚色/髮色
10. **AI 配色**：點擊「AI」按鈕（需先設定 Gemini API Key）進行 AI 上色，左右拖動分割條比較演算法與 AI 結果
11. **進階分析**：點擊「進階分析」展開場景偵測、診斷面板與自動校正項目
12. **下載結果**：點擊下載按鈕（AI 模式顯示為「下載 (AI)」）將配色後影像儲存為 PNG 格式

### 基本操作（Transfer 參考圖仿調色）

1. **選擇模式**：在首頁選擇中間的「參考圖仿調色」卡片
2. **上傳目標圖**：先將欲調色的圖片拖放至 Transfer 上傳區
3. **上傳參考圖**：工作區的中間欄會顯示「拖放參考圖」區塊，將風格參考圖拖放至此（或點擊上傳）
4. **查看結果**：系統自動執行三層管線並顯示目標／參考／結果三欄並列
5. **七項控制**：
   - **整體強度**：最終與原圖的混合比例
   - **光暗匹配**：Reinhard LAB 轉移中 L 通道強度
   - **色彩匹配**：Reinhard LAB 轉移中 a/b 通道強度
   - **色相對齊**：將目標主色相映射至參考主色相
   - **直方圖塑形**：對 L 通道做 Histogram Matching
   - **膚色保護**：保留膚色像素不變（同時作用於色相與 LAB）
   - **保留目標細節**：與 `overall` 相乘衰減，保留目標原圖紋理
6. **更換參考圖**：點擊右上角「更換參考圖」按鈕重新選擇
7. **AI 仿調色**：點擊「AI」按鈕執行 AI 仿調色，支援左右滑動比較
8. **進階分析**：展開後可查看參考圖場景、三方診斷對照、主色盤對照
...
```
pic-analysis/
├── index.html              # 主入口，三入口首頁（Recolor + Transfer + ToneLab）
├── test.html               # 單元測試套件
├── test/                   # 測試資源（影像素材）
├── css/
│   └── style.css           # 暗色主題樣式表（含 Recolor / Transfer 面板樣式）
├── js/
...
│   ├── recolor.js          # 智慧配色引擎（9 種方案 + 備選色相 + skinScore）
│   ├── transfer.js         # 參考圖仿調色引擎（Reinhard LAB + Histogram + Hue Alignment）
│   └── main.js             # 應用程式協調，UI 綁定（三入口 + 管線快取）
├── .claude/
...
| 演算法 | 用途 | 複雜度 |
|--------|------|--------|
| **K-means 分群** | 提取最多 6 種主要色彩（分析 / 配色調色盤 / Transfer Profile） | O(k × n × iter)，n ≤ 3000 |
...
| **RGB 色相旋轉矩陣** | Recolor 與 Transfer 共用的色相偏移 | O(1) per pixel |
| **Smoothstep 函式** | 多極配色方案的亮度分配過渡 | O(1) per pixel |
| **Reinhard LAB 轉移** | 參考圖仿調色的統計量對齊（mean/std） | O(n) |
| **Histogram Matching（CDF LUT）** | 亮度分布重塑至參考圖形狀 | O(n) + O(256²) |
| **Gaussian Hue Weighting** | 主色相映射的平滑融合（σ=0.10） | O(n × #anchors) |
...
| **Transfer Profile 快取** | 參考圖的 LAB 統計/CDF/調色盤算一次後快取於 `transferRefProfile`，滑桿拖動只重跑像素迴圈 |
| **Transfer 取樣** | 建 Profile / Source Stats 時最多抽 200,000 樣本計算統計與直方圖 |
| **去彈動畫** | Transfer 滑桿拖動時 `transferRerunTimer` 去彈，避免每次 input 重跑 |
| **非同步渲染** | 直方圖使用 `requestAnimationFrame` 避免阻塞 |
| **處理中遮罩** | 長時運算（AI 呼叫、初次管線）以 `#processing-overlay` 像素動畫提示 |
...
Recolor 引擎基於色彩和諧理論，提供 9 種經過精心設計的配色方案（含原始配色對照）。使用 RGB 空間色相旋轉確保色彩過渡平滑無色階，多極方案以亮度分配避免色相雜訊，膚色保護機制確保人像配色自然。備選色相方案（+120°/+240°）讓使用者快速探索不同色彩方向。智慧飽和度滑桿依處理模式自動估算初始值，手動拖動後凍結不再覆寫。AI 配色整合 Gemini 視覺模型，自動偵測並保留角色膚色與髮色。

**8. 以統計量對齊取代生成式推論（Reference Match Philosophy）**

Transfer（參考圖仿調色）模組的哲學是：大多數使用者想要「這張圖看起來像那張圖」的體驗，其實只是「色系對齊」，而非內容重繪。與其呼叫生成式 AI 重新繪製（昂貴、不可控、失去原圖細節），不如對齊 LAB 統計量、亮度直方圖與主色相——三者組合起來即可覆蓋絕大多數仿調色需求，且保留原圖的構圖、紋理與細節。七項獨立強度滑桿讓使用者可依場景微調各層貢獻（例如只要色彩、不要光暗，或只要色相、不要直方圖塑形）。

**9. 漸進式複雜度（Progressive Complexity）**

Recolor 與 Transfer 工作區皆將進階分析（場景偵測、診斷面板、校正項目、主色盤對照）預設收合，讓使用者專注於主要預覽與關鍵滑桿。需要深入了解時，展開進階分析即可。AI 功能同樣採漸進揭露——只有設定 API Key 後 AI 按鈕才啟用。

---

*本系統的哲學：最好的影像增強工具，是那個知道什麼時候不該動手的工具。最好的配色工具，是那個尊重色彩和諧與膚色自然的工具。最好的仿調色工具，是那個用統計量對齊、保留原圖細節、而非重新生成的工具。*
