...
> 一套純前端、無依賴的專業級影像分析引擎，提供**智慧調色分析**與**自動重新配色**雙模式。透過多階段管線自動診斷影像特性、識別藝術場景意圖，並以像素級精度施加保護性修正；亦可一鍵套用 8 種色彩和諧配色方案，搭配膚色保護與自動校正。
...
- **智慧調色分析（ToneLab）**：透過四階段管線自動診斷影像特性、識別藝術場景意圖，並以像素級精度施加保護性修正
- **自動重新配色（Recolor）**：提供 8 種色彩和諧配色方案，搭配膚色保護、強度控制與自動後校正
...
| **雙模式** | 智慧調色分析（ToneLab）與自動重新配色（Recolor）雙入口 |
...
ToneLab 採用**雙軌管線架構**——使用者從首頁選擇「智慧調色分析」或「自動重新配色」模式，兩條管線共享基礎模組（color.js、stats.js、lang.js）與診斷管線（Analyzer → Scene → Strategy → Adjuster），但 Recolor 管線額外引入配色引擎（recolor.js）。
...
獨立的進階功能模組，提供智慧色彩方案轉換，可將影像重新配色為 8 種色彩和諧方案之一。
...
PicAnalysis.Recolor.generateSchemes(imageData, hue?)  // 產生 8 種方案的預覽（含原始/新調色盤）
...
PicAnalysis.Recolor.SCHEMES                           // 8 種方案定義陣列
```
...
| 功能 | 說明 |
|------|------|
| **雙入口首頁** | 提供「智慧調色分析」與「自動重新配色」兩種模式選擇，各有獨立拖放區 |
| **影像載入** | 支援拖放（drag & drop）與點擊上傳，縮放至最大處理尺寸 |
| **管線執行** | ToneLab 模式：Analyzer → Scene → Strategy → Adjuster |
| **配色管線** | Recolor 模式：產生 8 種配色方案、膚色保護、自動後校正 |
| **處理模式切換** | 攝影模式 ↔ 插畫模式即時切換，參數與場景修飾表同步更新 |
...
1. **選擇模式**：在首頁選擇「自動重新配色」卡片
2. **上傳影像**：將圖片拖放至 Recolor 上傳區
3. **瀏覽方案**：使用左右箭頭或方案點選按鈕瀏覽 8 種配色方案
4. **調整強度**：使用強度滑桿（0-100%）控制配色效果的混合比例
5. **膚色保護**：使用膚色保護滑桿（0-100%）控制膚色區域的保護力度
6. **自訂基色相**：使用基色相滑桿（0-360°）手動指定配色基準色相，或按「Auto」自動偵測
7. **查看對比**：色盤對比區顯示原始色盤與新配色方案的色彩對照
8. **查看後校正**：下方顯示自動後校正的診斷面板與調整列表
9. **下載結果**：點擊下載按鈕將配色後影像儲存為 PNG 格式
...
// 執行 Recolor 配色
const schemes = PicAnalysis.Recolor.generateSchemes(imageData); // 8 種方案
...
Recolor 引擎基於色彩和諧理論，提供 8 種經過精心設計的配色方案。使用 RGB 空間色相旋轉確保色彩過渡平滑無色階，多極方案以亮度分配避免色相雜訊，膚色保護機制確保人像配色自然。
